...
from .hyp_utils import set_seed, PoisonDataset, make_and_restore_cifar_model
...
def hypocritical():
    ...
    parser.add_argument('--train_loss', default='AT', type=str, choices=['ST', 'AT', 'TRADES', 'MART'])
    ...
    poison_trainset = list([torch.zeros(3, 32, 32), 0] for _ in range(50000))
    for i in range(len(poison_trainset)):
        poison_trainset[i][0] = poison_data[0][i]
        poison_trainset[i][1] = poison_data[1][i]
