...
def _ntga(targs):
    ...
    if targs.dataset == "CIFAR-10":
        width = 32
        x_train_adv = x_train_adv.reshape(-1, width, width, 3)
    ...
    train_data = np.load('{:s}x_train_{:s}_ntga_{:s}.npy'.format(args.save_path, args.dataset, args.model_type), x_train_adv)
    train_label = np.load('{:s}y_train_{:s}_ntga_{:s}.npy'.format(args.save_path, args.dataset, args.model_type), y_train_adv)

    poison_trainset = list([torch.zeros(3, width, width), 0] for _ in range(len(train_data)))

    for i in range(len(poison_trainset)):
        poison_trainset[i][0] = torch.from_numpy(train_data[i]).permute(2,0,1)
        poison_trainset[i][1] = torch.topk(torch.from_numpy(train_label[i]),1)[1].squeeze(0).item()
    poison_trainset = np.load('{:s}x_train_{:s}_ntga_{:s}.npy'.format(args.save_path, args.dataset, args.model_type), x_train_adv)
